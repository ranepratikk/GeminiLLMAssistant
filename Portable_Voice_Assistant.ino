...
// ---------------- Wake Word (WakeNet) support ----------------
// This is an OPTIONAL module.
// If your Arduino setup does not have ESP-SR / esp-skainet installed, keep ENABLE_WAKENET = 0.
//
// What it does:
// - ESP32 listens continuously for a wake word (e.g. "Hey Wanda")
// - Once detected, it automatically starts recording
//
// ⚠️ Reality check for interviews:
// Arduino-IDE + ESP32 classic boards may not support ESP-SR WakeNet easily.
// This integration is written to be cleanly switchable ON/OFF.

#define ENABLE_WAKENET 0

// WakeNet GPIO / behavior
#define WAKE_AUTORECORD_SECONDS 4  // auto stop record after wake-trigger (seconds)

#if ENABLE_WAKENET
  // ESP-SR headers (available in ESP-IDF / some Arduino cores if installed)
...
// ========================= WakeNet config =========================
// 0 = disabled (manual record button only)
// 1 = enabled  (hands-free wake word)
#define ENABLE_WAKENET 0

// "Hey Wanda" is usually added via ESP-SR WakeNet custom model training.
// In code, we treat any detected wake as "Hey Wanda" trigger.

#if ENABLE_WAKENET
  // These headers belong to ESP-SR (Espressif Speech Recognition) / esp-skainet
  // Install ESP-SR components or use ESP-IDF + Arduino as component.
  // If compilation fails, set ENABLE_WAKENET to 0.
  #include "esp_afe_sr_models.h"
  #include "esp_afe_sr_iface.h"
  #include "esp_mn_iface.h"
  #include "esp_mn_models.h"
  ...
  static const esp_mn_iface_t *wakenet_iface = NULL;
  static model_iface_data_t *wakenet_model = NULL;

  // WakeNet tuning
  #define WAKENET_SAMPLE_RATE 16000
  #define WAKENET_CHANNELS 1
  #define WAKENET_TRIGGER_HOLD_MS 1500

  volatile bool g_wake_triggered = false;
  uint32_t g_wake_triggered_at = 0;
